  ...
  return (
    <section className="content-view" aria-labelledby="about-title">
      ...
      <div className="content-grid about-grid">
        <article className="about-card glass-panel"><p className="eyebrow">The project</p><h2 className="panel-title">A working engineering software prototype</h2><div className="about-copy"><p>The visual factory models two enclosed horizontal band saws feeding a shared dual-lane central conveyor, two six-machine CNC rows staffed at one operator per two machines, front-conveyor return flow, direct robotic CMM loading, and coordinate inspection. The live instrumented path follows two CNC operations while traceable parts move through constrained stations and machine health shapes throughput and quality.</p><p>FactoryOS combines manufacturing systems thinking with modern frontend engineering. Its dashboard does not display decorative random values: OEE and every supporting metric are calculated from the cell&apos;s accumulated runtime, cycle, and inspection data.</p></div><div className="tech-badges">{tech.map((item) => <span className="tech-badge" key={item}>{item}</span>)}</div></article>
