...
export function InspectionStation() {
  ...
  const selected = useFactoryStore((state) => state.selectedMachineId === "CMM-01");
  const select = useFactoryStore((state) => state.selectMachine);
  const probe = useRef<Mesh>(null);
  useFrame(() => { if (probe.current) probe.current.position.y = 2.25 - Math.sin(machine.progress/100*Math.PI)*.65; });
  const click = (event: ThreeEvent<MouseEvent>) => { event.stopPropagation(); select("CMM-01"); };
  return (
    <group position={[7.7,0,2.2]} onClick={click} userData={{ machineId:"CMM-01" }}>
      ...
      <mesh position={[-.78,3.32,0]}><sphereGeometry args={[.11,10,8]} /><meshStandardMaterial color={colors[machine.status]} emissive={colors[machine.status]} emissiveIntensity={2.4} /></mesh>
      ...
      <MachineLabel id="CMM-01" status={machine.status} position={[0,3.85,0]} />
    </group>
  );
}
