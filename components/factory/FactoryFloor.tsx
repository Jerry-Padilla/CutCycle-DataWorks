...
function SafetyBarrier() {
  const posts: [number, number, number][] = [[7.2,1.1,7.2],[9.1,1.1,7.2],[11,1.1,7.2],[12.9,1.1,7.2]];
  return (
    <group>
      ...
      {[8.15,10.05,11.95].map((x) => <mesh key={x} position={[x,1.1,7.2]}><planeGeometry args={[1.8,2]} /><meshPhysicalMaterial color="#8bc6d8" transparent opacity={.1} side={THREE.DoubleSide} roughness={.12} transmission={.2} /></mesh>)}
    </group>
  );
}
